...
def fit_trial(self, phi, W):
    ...
    res1 = minimize(fun = cost_fun_for_W, x0 = W, args = (self.endog, self.exog, phi), bounds = bounds_W, method = self.method, options = {'maxiter': 50 * self.exog.shape[1]}) 
    W = res1.x
    bounds_phi = [(0, 1)]
    res2 = minimize(fun = cost_fun_for_phi, x0 = phi, args = (self.endog, self.exog, W), bounds = bounds_phi, method = self.method, options = {'maxiter': 10}) 
